			        ...
			        if not opInfo then
						...
						elseif opName == "GETIMPORT" then
						    ...
						    if bit32_band and bit32_rshift then
						        ...
						        local function clean(idx)
						            local s = self:getConstant(idx)
						            return (s:sub(1,1) == "\"" and s:sub(-1,-1) == "\"") and s:sub(2, -2) or s
						        end
						
						        path = clean(id0)
						        if count > 1 then path = path .. "." .. clean(bit32_band(bit32_rshift(aux, 10), 0x3FF)) end
						        if count > 2 then path = path .. "." .. clean(bit32_band(aux, 0x3FF)) end
						    end
