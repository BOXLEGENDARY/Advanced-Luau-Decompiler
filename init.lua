	...
	local function finalize(mainProtoId, registerActions, protoTable)
		...
		if DECOMPILER_MODE == "disasm" then -- disassembler
			...
			function Context:getDebugLocalName(index)
				if self.debugLocals then
					for _, loc in ipairs(self.debugLocals) do
						if loc.register == index and self.pc >= loc.startPC and self.pc <= loc.endPC then
							return loc.name
						end
					end
				end
			...
			function Context:process()
				...
				while self.pc <= self.length do
					...
					elseif opName == "DUPCLOSURE" then
						local constVal = self.constants[D + 1].value
						self:setReg(A, "func_proto_" .. constVal, PREC.ATOMIC)
					end
			...
			function Lifter.decompile(proto, protoId, depth)
				...
				if not ok then return "-- Decompiler Fatal Error: \n-- " .. tostring(res) .. "\n" .. table_concat(ctx.lines, "\n") end
				return res
			end
