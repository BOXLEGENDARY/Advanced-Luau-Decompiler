...
local function Decompile(bytecode)
	...
	local function finalize(mainProtoId, registerActions, protoTable)
		...
		else -- assume optdec - optimized decompiler ( It's just temporary; if you want to improve it, go ahead. )
			local Lifter = {}
			    ...
			    while self.pc <= self.length do
			        ...
			        elseif opName == "NEWCLOSURE" then
			            ...
			            if closureData then
			                ...
			                self:indent()
			                for line in string.gmatch(closureData.code, "[^\r\n]+") do self:emit(line) end
			                self:outdent()
			                self:emit("end")
			                self:emit("")
			                ...
			                self:emit("--[[ Error: Closure " .. protoIdx .. " not found! ]]")
			            end
			        elseif opName == "DUPCLOSURE" then
			            local constVal = self.constants[D + 1].value
			            self:assignReg(A, "func_proto_" .. constVal)
			        end
