		...
		if DECOMPILER_MODE == "disasm" then -- disassembler
			...
			function Context:process()
			    ...
			    while self.pc <= self.length do
			        ...
			        local i = #self.scopeStack
			        while i > 0 do
			            local scope = self.scopeStack[i]
			            if pc >= (scope.endPC or math.huge) then
			                self:outdent()
			                
			                if scope.type == "IF" and self.instructions[pc - 1] then
			                    local p_ok, p_op = pcall(function() return Luau:INSN_OP(self.instructions[pc - 1]) end)
			                    local p_info = p_ok and LuauOpCode[p_op]
			                    if p_info and p_info.name == "JUMP" then
			                        local p_sD = 0
			                        pcall(function() p_sD = Luau:INSN_sD(self.instructions[pc - 1]) end)
			                        local elseEnd = (pc - 1) + 1 + p_sD
			                        if elseEnd > pc then
			                            self:emit("else")
			                            self:indent()
			                            scope.type = "ELSE"
			                ...
			                if scope.type ~= "ELSE" or pc >= scope.endPC then
			                    self:emit("end")
			        ...
			        if not opInfo then
			            ...
			            elseif opName == "NAMECALL" then
			                self.pendingNamecall = { obj = self:getReg(B, PREC.ATOMIC), method = self:getConstant(aux), reg = A }
			            ...
			            elseif opName == "NEWCLOSURE" then
			                local closureData = self.compiledClosures[D]
			                if closureData then
			                    self:emit("\nlocal function " .. closureData.name .. "(...)")
			                    self:indent()
